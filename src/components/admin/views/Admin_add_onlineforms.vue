    ...
    <div class="container">
         ...
         <div class="card">
            <div class="card-header">
                <i class="fa fa-database"></i> Add New SMTP
            </div>
            <div class="card-body">
                    <form @submit.prevent="saveonlineforms()">
                        <div class="form-row">
     ...
     if(this.frm.mail_method =="PHPMAIL" && this.frm.smtp_server =="N/A" && (this.frm.smtp_user == "" && this.frm.smtp_password =="" ) ){
       ...
       if(res.length == 0){
           ...
           if(con){
               ...
               this.$store.dispatch("onlineforms/save_form", fdata).then((resp)=>{
                   if(resp.code == 200){
                       this.showMsg = true;
                        this.hasErr = false;
                        this.alertMsg = "Save successfully!"
                       setTimeout(() => {
                        this.clearTxt()
                      }, 2000);  
                   }else{
